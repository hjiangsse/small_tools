...
func NsqdFlagSet(opts *nsqd.Options) *flag.FlagSet {
	...
	flagSet.Int64("resend-queue-size", opts.ResendQueueSize, "number of messages to keep in memory for resending (per channel)")
	flagSet.Int64("mem-ring-size-pow", opts.MemRingSizePow, "number of messages to keep in topic memory ring buffer (based on the power of 2)")
